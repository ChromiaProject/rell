...
class SystemLibVisitor(
    ...
    fun visitRellModule(rellModule: RellModule): List<DPackage> {
        ...
        val functions = namespaceMembers.filterIsInstance<L_NamespaceMember_Function>()
                .visitFunctions(dri)
        val specialFunctions = namespaceMembers.filterIsInstance<L_NamespaceMember_SpecialFunction>()
                .visitSpecialFunctions(dri)
        ...
        val basePackage = DPackage(
                ...
                functions = functions + specialFunctions
        )
    ...
    private fun L_NamespaceMember_SpecialFunction.visit(parent: DRI): DFunction {
        return when (simpleName.str) {
            "exists" -> existsAndEmptySpecialFunctions(this, sourceSet)
            "empty" -> existsAndEmptySpecialFunctions(this, sourceSet)
            else -> TODO("Special $simpleName function not treatet")
        }
    }
    ...
    private fun List<L_NamespaceMember_SpecialFunction>.visitSpecialFunctions(parent: DRI) = map { it.visit(parent) }
