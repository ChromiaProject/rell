...
import net.postchain.rell.performance.benchmarks.RellBenchmarkBase
 ...
 *
 * This branch's port runs the legacy R_Expr tree-walker (no RR_App, no Truffle on master).
 * The `--backend` option is accepted but only `r-expr-legacy` (== legacy interpreter) works.
 */
class ProfileSampleCommand : CliktCommand(name = "profile-sample") {
    ...
    private val backend by option(
        "--backend", help = "Runtime backend: r-expr-legacy (the only backend on this branch).",
    ).default("r-expr-legacy")
    ...
    private val warmups by option(
        "--warmups",
        help = "Pre-profile invocations to warm up the JIT (default: 30).",
    ).int().default(30)
    ...
    override fun run() {
        check(backend == "r-expr-legacy") {
            "This branch only supports backend=r-expr-legacy (legacy R_Expr tree-walker). " +
                "Truffle and RR_App backends are dev-only."
        }
        ...
        val app = harness.loadApp(resourcePath)
        val module = app.moduleMap.getValue(R_ModuleName.EMPTY)
        val q = module.queries[queryName]
            ?: error("Query `$queryName` not found in $resourcePath. Available: " +
                module.queries.keys.joinToString())
        val args: List<Rt_Value> = listOf(Rt_IntValue.get(argValue))

        // Warm-up phase — do not start the profiler until JIT has settled.
        log("profile-sample", "warmup × $warmups...")
        repeat(warmups) { ProfileSampleHotLoop.runOnce(q, harness.exeCtx, args) }
        ...
        repeat(reps) { ProfileSampleHotLoop.runOnce(q, harness.exeCtx, args) }
...
/** Tiny visible-from-outside subclass of [RellBenchmarkBase] used by the CLI. */
private class SampleHarness : RellBenchmarkBase() {
    fun loadApp(resourcePath: String) = setUpApp(resourcePath)
}
