...
val buildNativeLibrary by tasks.registering(Exec::class) {
    ...
    // One shared object; clang++ compiles + links all TUs in a single -shared call. jni_bridge.cpp
    // stays first (it owns JNI_OnLoad / g_vm). The rest is the ABI fileList: tagged-value
    // marshalling (value.cpp), the JNI stdlib/SQL back-call bridges, the pure-family intrinsics,
    // and the RR-tree lowering passes.
    val cppSources = listOf(
        ...
        "lower_stmt.cpp",
        "lower_call.cpp",
        // Native numerics: faithful OpenJDK BigInteger/BigDecimal port + Rell decimal-math
        // semantics (rell::num). Standalone of JNI/LLVM; the runtime glue that makes the
        // intrinsics call these instead of the JVM is wired separately. (rell_numerics_test.cpp
        // is the standalone unit test — deliberately NOT linked into the shared lib.)
        "rell_bigint_core.cpp",
        "rell_bigint_muldiv.cpp",
        "rell_bigint_extra.cpp",
        "rell_bigdec_core.cpp",
        "rell_bigdec_muldiv.cpp",
        "rell_bigdec_str.cpp",
        "rell_decimal_math.cpp",
    ...
    // Track the whole cpp dir so an edit to any source or the shared rell_runtime.h rebuilds.
    inputs.dir(cppDir)
