...
object L_TypeUtils {
    ...
    fun getRTypeOrNull(mType: M_Type): R_Type? = when (mType) {
    ...
    private fun getRTypeForTuple(mType: M_Type_Tuple): R_TupleType? {
        val rFields = mType.fieldTypes.indices.mapNotNullAllOrNull { i ->
            ...
            if (rFieldType == null) null else {
                ...
                val rName = if (mName == null) null else R_IdeName(Name.of(mName), C_IdeSymbolInfo.MEM_TUPLE_ATTR)
    ...
    /** Derive [R_TypeMeta] from a simple (non-parameterized) [RR_Type]. Used by the library DSL. */
    fun rrTypeToRTypeMeta(rrType: RR_Type): R_TypeMeta {
        val rType = rrTypeToRType(rrType)
        return R_TypeMeta.mkSimple(rType)
    }

    fun rrTypeToRType(rrType: RR_Type): R_Type = when (rrType) {
        is RR_Type.Primitive -> when (rrType.kind) {
            RR_PrimitiveKind.BOOLEAN -> R_BooleanType
            RR_PrimitiveKind.INTEGER -> R_IntegerType
            RR_PrimitiveKind.BIG_INTEGER -> R_BigIntegerType
            RR_PrimitiveKind.DECIMAL -> R_DecimalType
            RR_PrimitiveKind.TEXT -> R_TextType
            RR_PrimitiveKind.BYTE_ARRAY -> R_ByteArrayType
            RR_PrimitiveKind.ROWID -> R_RowidType
            RR_PrimitiveKind.GUID -> R_GUIDType
            RR_PrimitiveKind.SIGNER -> R_SignerType
            RR_PrimitiveKind.JSON -> R_JsonType
