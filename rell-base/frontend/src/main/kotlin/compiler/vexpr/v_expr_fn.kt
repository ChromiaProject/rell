...
sealed class V_CommonFunctionCall(
    ...
    protected abstract fun dbExprWhatCombinerInfo(rTarget: R_FunctionCallTarget, safe: Boolean): Db_AtWhatCombinerInfo
    ...
    fun dbExprWhat(base: V_Expr?, safe: Boolean): C_DbAtWhatValue {
        ...
        val info = dbExprWhatCombinerInfo(rTarget, safe)

        val evaluator = object: Db_ComplexAtWhatEvaluator() {
            override fun combinerInfo() = info
        }
...
class V_CommonFunctionCall_Partial(
    ...
    override fun dbExprWhatCombinerInfo(rTarget: R_FunctionCallTarget, safe: Boolean): Db_AtWhatCombinerInfo {
        return Db_AtWhatCombinerInfo.PartialFunctionCall(returnType, rTarget, mapping)
    }
}
...
class V_FunctionCallTarget_NativeUserFunction(
        private val fnName: FullName,
        private val typesGetter: C_LateGetter<Pair<ImmList<R_Type>, R_Type>>,
): V_FunctionCallTarget() {
    override fun toRTarget(): R_FunctionCallTarget {
        val (argTypes, resultType) = typesGetter.get()
        return R_FunctionCallTarget_NativeUserFunction(fnName, argTypes, resultType)
    }
...
data object V_FunctionCallTarget_FunctionValue: V_FunctionCallTarget() {
    override fun toRTarget(): R_FunctionCallTarget = R_FunctionCallTarget_FunctionValue
...
abstract class V_FunctionCall(
        val argIdeInfos: ImmMap<Name, C_IdeSymbolInfo>,
)

class V_GlobalFunctionCall(
        private val expr: V_Expr,
        val ideInfo: C_IdeSymbolInfo? = null,
        argIdeInfos: ImmMap<Name, C_IdeSymbolInfo>,
...
abstract class V_MemberFunctionCall(
        ...
        paramIdeInfos: ImmMap<Name, C_IdeSymbolInfo>,
    ...
    open fun lower(ctx: C_ExprContext, base: V_Expr): V_Expr? = null
...
class V_MemberFunctionCall_CommonCall(
    ...
    class R_MemberCalculator_CommonCall(
        type: R_Type,
