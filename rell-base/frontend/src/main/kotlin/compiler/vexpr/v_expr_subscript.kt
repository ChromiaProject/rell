...
private val DB_TEXT_SUBSCRIPT = Db_SysFunction.template("text.[]", 2, "${R_SqlConstants.FN_TEXT_GETCHAR}(#0, (#1)::INT)")
private val DB_BYTE_ARRAY_SUBSCRIPT = Db_SysFunction.template("byte_array.[]", 2, "GET_BYTE(#0, (#1)::INT)")
...
data object V_CommonSubscriptKind_Text: V_CommonSubscriptKind(R_TextType) {
    ...
    override fun compileDb(pos: S_Pos, dbBase: Db_Expr, dbKey: Db_Expr): Db_Expr {
        return Db_CallExpr(R_TextType, DB_TEXT_SUBSCRIPT, immListOf(dbBase, dbKey))
    }
}

data object V_CommonSubscriptKind_JsonArray: V_CommonSubscriptKind(R_JsonType) {
    override fun compileR(pos: S_Pos, rBase: R_Expr, rKey: R_Expr): R_Expr {
        val errPos = pos.toErrorPos()
    ...
    override fun compileDb(pos: S_Pos, dbBase: Db_Expr, dbKey: Db_Expr): Db_Expr {
        return Db_CallExpr(
            R_JsonType,
            Db_SysFunction.simple("json[integer]", R_SqlConstants.FN_JSON_ARRAY_GET),
            immListOf(dbBase, dbKey),
        )
    }
}

data object V_CommonSubscriptKind_JsonObject: V_CommonSubscriptKind(R_JsonType) {
    override fun compileR(pos: S_Pos, rBase: R_Expr, rKey: R_Expr): R_Expr {
        val errPos = pos.toErrorPos()
    ...
    override fun compileDb(pos: S_Pos, dbBase: Db_Expr, dbKey: Db_Expr): Db_Expr {
        return Db_CallExpr(
            R_JsonType,
            Db_SysFunction.simple("json[text]", R_SqlConstants.FN_JSON_OBJECT_GET),
            immListOf(dbBase, dbKey),
        )
    }
}

data object V_CommonSubscriptKind_ByteArray: V_CommonSubscriptKind(R_IntegerType) {
    override fun compileR(pos: S_Pos, rBase: R_Expr, rKey: R_Expr): R_Expr {
        val errPos = pos.toErrorPos()
    ...
    override fun compileDb(pos: S_Pos, dbBase: Db_Expr, dbKey: Db_Expr): Db_Expr {
        return Db_CallExpr(R_IntegerType, DB_BYTE_ARRAY_SUBSCRIPT, immListOf(dbBase, dbKey))
    }
}
...
data object V_TupleSubscriptKind_Simple: V_TupleSubscriptKind() {
    override fun compile(resType: R_Type, index: Int) = R_MemberCalculator_TupleAttr(resType, index)
}

data object V_TupleSubscriptKind_Virtual: V_TupleSubscriptKind() {
