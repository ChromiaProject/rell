...
abstract class S_Statement(val startPos: S_Pos, val endPos: S_Pos) {
    private val modifiedVars = TypedKey<Set<Name>>()

    abstract fun compile(ctx: C_StmtContext, repl: Boolean): C_Statement

    fun compileSafe(ctx: C_StmtContext, repl: Boolean = false): C_Statement {
    ...
    fun getModifiedVars(ctx: C_FunctionContext): Set<Name> {
        val res = ctx.statementVars[modifiedVars]
        return res
    }
...
sealed class S_VarDeclarator {
    abstract fun discoverVars(vars: MutableSet<Name>)
...
class S_SimpleVarDeclarator(
    ...
    override fun discoverVars(vars: MutableSet<Name>) {
