...
private class C_FunctionCompilerBase(
    ...
    fun definitionBase(ideKind: IdeSymbolKind, docCommentGetter: C_LateGetter<DocComment?>): C_UserDefinitionBase {
        ...
        return comBase.userBase(sFn.fnPos, mntCtx.executor)
    }
}
...
private abstract class C_FunctionCompiler(
    ...
    private val docCommentLate = base.executor.lateInit(C_CompilerPass.EXPRESSIONS, null as DocComment?)
...
private class C_FunctionCompiler_Regular(
    ...
    override fun compile(defCtx: C_DefinitionContext, ideCompsLate: C_LateInit<ImmMultimap<String, IdeCompletion>>) {
        ...
        val rFnBase = R_FunctionBase(defCtx.executor, rDefBase.defName)
        val rFn = R_FunctionDefinition(rDefBase, rFnBase, base.isTest, base.disabled)
        val cFn = C_RegularUserGlobalFunction(defCtx.executor, rFn, null)
...
private class C_FunctionCompiler_Abstract(
    ...
    override fun compile(defCtx: C_DefinitionContext, ideCompsLate: C_LateInit<ImmMultimap<String, IdeCompletion>>) {
        ...
        val rFnBase = R_FunctionBase(defCtx.executor, rDefBase.defName)
        val rFn = R_FunctionDefinition(rDefBase, rFnBase, base.isTest, base.disabled)
        val cFn = C_AbstractUserGlobalFunction(defCtx.executor, fnPos, rFn, sFn.body != null, rFnBase)
...
private class C_FunctionCompiler_Override(
    ...
    override fun compile(defCtx: C_DefinitionContext, ideCompsLate: C_LateInit<ImmMultimap<String, IdeCompletion>>) {
        ...
        val rFnBase = R_FunctionBase(defCtx.executor, defCtx.defName)
        val descriptor = C_OverrideFunctionDescriptor(defCtx.executor, fnPos, rFnBase)
...
private class C_FunctionCompiler_Native(
    ...
    override fun compile(defCtx: C_DefinitionContext, ideCompsLate: C_LateInit<ImmMultimap<String, IdeCompletion>>) {
        ...
        val conversionsLate = defCtx.lateInit(C_CompilerPass.MEMBERS, Pair(immListOf<R_Type>(), R_UnitType as R_Type))

        val rDefBase = cDefBase.rBase(defCtx.initFrameGetter)
        val rFnBase = R_FunctionBase(defCtx.executor, rDefBase.defName)
        val rFn = R_FunctionDefinition(rDefBase, rFnBase, base.isTest, base.disabled)
        val cFn = C_NativeUserGlobalFunction(defCtx.executor, rFn, fullName, conversionsLate.getter)
    ...
    private fun compileAdapters(
        defCtx: C_DefinitionContext,
        header: C_UserFunctionHeader,
    ): Pair<ImmList<R_Type>, R_Type> {
        val argTypes = header.params.list.mapToImmList { param ->
            validateNativeType(defCtx, param.type, param.name.pos, "parameter")
        }
        val resultType = validateNativeType(defCtx, header.explicitType ?: R_UnitType, fnPos, "result")
        return Pair(argTypes, resultType)
    }

    private fun validateNativeType(
        defCtx: C_DefinitionContext,
        type: R_Type,
        pos: S_Pos,
        kind: String,
    ): R_Type {
        val hasNativeConversion = hasNativeConversion(type)

        if (!hasNativeConversion) {
    ...
    private fun hasNativeConversion(type: R_Type): Boolean = when (type) {
        is R_IntegerType, is R_BooleanType, is R_TextType, is R_ByteArrayType,
        is R_DecimalType, is R_BigIntegerType, is R_RowidType, is R_UnitType,
        is R_GtvType -> true
        is R_NullableType -> hasNativeConversion(type.valueType)
        else -> false
    }
}
