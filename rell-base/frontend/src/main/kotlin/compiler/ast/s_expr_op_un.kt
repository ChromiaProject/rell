...
internal data object S_UnaryOp_Minus: S_UnaryOp("-") {
    override fun compile(ctx: C_ExprContext, startPos: S_Pos, opPos: S_Pos, expr: V_Expr): V_Expr {
        val vOp = when (val type = expr.type) {
            R_IntegerType -> V_UnaryOp_Minus(type, R_UnaryOp.Minus_Integer, Db_UnaryOp_Minus_Integer)
            R_BigIntegerType -> V_UnaryOp_Minus(type, R_UnaryOp.Minus_BigInteger, Db_UnaryOp_Minus_BigInteger)
            R_DecimalType -> V_UnaryOp_Minus(type, R_UnaryOp.Minus_Decimal, Db_UnaryOp_Minus_Decimal)
            else -> {
                errTypeMismatch(ctx, opPos, type)
                V_UnaryOp_Minus(type, R_UnaryOp.Minus_Integer, Db_UnaryOp_Minus_Integer) // Fake op for error recovery.
            }
        }
...
internal class S_UnaryOp_IncDec(val inc: Boolean, val post: Boolean): S_UnaryOp(if (inc) "++" else "--") {
    ...
    companion object {
        private val INTEGER_ONE = R_RRConstantValueExpr(R_IntegerType, RR_ConstantValue.Int(1))
        private val BIG_INTEGER_ONE = R_RRConstantValueExpr(R_BigIntegerType, RR_ConstantValue.BigInteger("1"))
        private val DECIMAL_ONE = R_RRConstantValueExpr(R_DecimalType, RR_ConstantValue.Decimal("1"))
