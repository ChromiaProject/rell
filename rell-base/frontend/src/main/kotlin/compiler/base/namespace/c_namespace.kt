...
sealed class C_Namespace {
    abstract fun getEntries(): ImmMap<Name, C_NamespaceEntry>
    abstract fun getEntry(name: Name): C_NamespaceEntry?
...
private class C_BasicNamespace(private val entries: ImmMap<Name, C_NamespaceEntry>): C_Namespace() {
    ...
    override fun getEntries(): ImmMap<Name, C_NamespaceEntry> {
        return entries
    }

    override fun getEntry(name: Name): C_NamespaceEntry? {
        return entries[name]
    }
...
private class C_LateNamespace(private val getter: () -> C_Namespace): C_Namespace() {
    ...
    override fun getEntry(name: Name) = getter().getEntry(name)
    override fun getDocMembers() = getter().getDocMembers()
}

class C_NamespaceBuilder {
    private val directMembers = mutableMultimapOf<Name, C_NamespaceMember>()
    private val importMembers = mutableMultimapOf<Name, C_NamespaceMember>()

    fun add(name: Name, member: C_NamespaceMember) {
        directMembers.put(name, member)
    }

    fun add(name: Name, entry: C_NamespaceEntry) {
