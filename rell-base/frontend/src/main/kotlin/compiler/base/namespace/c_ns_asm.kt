...
import net.postchain.rell.base.model.AppUid
...
class C_NsAsm_WildcardImport(
    val module: C_ModuleKey,
    val path: ImmList<C_Name>,
    val names: C_NsAsm_WildcardImportNames,
)

class C_NsAsm_ExactImport(
    val module: C_ModuleKey,
...
class C_NsAsm_Namespace(
        val defs: ImmMap<Name, C_NsAsm_Def>,
        val importDefs: ImmMultimap<Name, C_NsAsm_Def>,
...
private class C_NsAsm_Def_LateNamespace(
    ...
    override fun ns(): C_NsAsm_Namespace = getter()
    override fun ideSymbolInfo(): C_IdeSymbolInfo = ideInfo
}
...
private class C_NsAsm_InternalReplAssembler(
    ...
    private fun mergeReplNamespace(oldNs: C_NsImp_Namespace, newNs: C_NsImp_Namespace): C_NsImp_Namespace {
        ...
        val importDefs = mutableMultimapOf<Name, C_NsImp_Def>()
...
class C_NsAsm_RawNamespace(
    ...
    fun filterByStamp(stamp: AppUid): C_NsAsm_RawNamespace {
    ...
    fun assemble(sysEntries: List<C_NsEntry>): C_NsAsm_Namespace {
        val defs = mutableMapOf<Name, C_NsAsm_Def>()
    ...
    private data class ImportKey(val module: C_ModuleKey, val path: ImmList<Name>)
...
class C_NsAsm_RawEntry(
    ...
    fun filterByStamp(targetStamp: AppUid): C_NsAsm_RawEntry? {
...
sealed class C_NsAsm_RawDef {
    abstract fun filterByStamp(targetStamp: AppUid): C_NsAsm_RawDef
...
private class C_NsAsm_RawDef_Simple(private val member: C_NamespaceMember): C_NsAsm_RawDef() {
    override fun filterByStamp(targetStamp: AppUid) = this
...
private class C_NsAsm_RawDef_Namespace(
    ...
    override fun filterByStamp(targetStamp: AppUid): C_NsAsm_RawDef {
...
private class C_NsAsm_ConflictsProcessor(
    ...
    private val sysDefs: ImmMap<Name, C_DeclarationType> = let {
        val mutSysDefs = mutableMapOf<Name, C_DeclarationType>()
    ...
    private fun report(name: C_Name, entryStamp: AppUid, otherType: C_DeclarationType, otherName: C_Name?) {
...
private data class C_NsAsm_NamespaceKey(val container: C_ContainerKey, val path: C_RNamePath = C_RNamePath.EMPTY) {
    fun sub(name: Name): C_NsAsm_NamespaceKey {
...
private object C_NsAsm_Utils {
    fun mergeNamespaces(ns: C_NsAsm_RawNamespace, stamp: AppUid): C_NsAsm_RawNamespace {
        return transformNamespaces(ns, true, stamp) { oldNs, _ ->
            mergeNamespaces0(oldNs, stamp)
        }
    }

    private fun mergeNamespaces0(ns: C_NsAsm_RawNamespace, stamp: AppUid): C_NsAsm_RawNamespace {
