...
class C_GlobalConstantDefinition(
    ...
    companion object {
        ...
        private fun validateRecursion(msgCtx: C_MessageContext, constants: List<C_GlobalConstantDefinition>) {
            class Edge(val srcId: GlobalConstantId, val tgtExpr: V_Expr)
...
class C_GlobalConstantDeepDefinitionBody(
    ...
    private var constantValue0: One<RR_ConstantValue?>? = null
    ...
    fun constantValue(ctx: V_ConstantValueEvalContext): RR_ConstantValue? {
        val o = constantValue0
        if (o != null) {
            return o.value
        }

        var value: RR_ConstantValue? = null
    ...
    private fun calcConstantValue(ctx: V_ConstantValueEvalContext): RR_ConstantValue? {
        val vExpr = compile()
        return try {
            ...
            val msg = e.message
            if (msg != null) {
                defCtx.msgCtx.error(vExpr.pos, e.javaClass.simpleName, msg)
            }
            null
        }
    }
