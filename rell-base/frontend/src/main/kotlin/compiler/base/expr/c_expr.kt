...
abstract class C_Expr {
    ...
    fun vExprOrNull(msgCtx: C_MessageContext): V_Expr? = when (val res = vExprOrError()) {
    ...
    fun vExpr(): V_Expr = when (val res = vExprOrError()) {
        is C_ValueOrError_Value -> res.value
...
class C_ValueMemberExpr(
    ...
    override fun call(ctx: C_ExprContext, pos: S_Pos, args: S_CallArguments, resTypeHint: C_TypeHint): C_Expr {
        ...
        val lowered = vMember.lower(exprCtx, vBase)
        if (lowered != null) return C_ValueExpr(lowered)
