...
class C_NamespaceContext(
    ...
    fun getFullName(simpleName: Name): FullName {
        ...
        return FullName(modCtx.moduleName, qualifiedName)
    }
...
class C_DefinitionModuleName(val module: String, val chain: String? = null) {
    constructor(module: ModuleName): this(module.str())

    fun str(): String = if (chain == null) module else "$module[$chain]"
    override fun toString() = str()
}

class C_DefinitionName(
    val module: C_DefinitionModuleName,
    val qualifiedName: C_StringQualifiedName,
) {
    constructor(module: String, name: String): this(
        C_DefinitionModuleName(module),
        C_StringQualifiedName.of(name),
    )

    constructor(module: String, qualifiedName: C_StringQualifiedName): this(
        C_DefinitionModuleName(module),
        qualifiedName,
    )

    constructor(fullName: FullName): this(
        fullName.moduleName.str(),
        C_StringQualifiedName.of(fullName.qualifiedName),
    )

    val appLevelName: String by lazy { DefinitionName.appLevelName(module.str(), qualifiedName.str()) }

    fun toRDefName(): DefinitionName {
        val qName = qualifiedName.str()
        return DefinitionName(module.str(), qName, qualifiedName.last)
    }
...
class C_DefinitionPath(val module: C_DefinitionModuleName, val path: ImmList<String>) {
    ...
    constructor(module: ModuleName, path: ImmList<String>): this(C_DefinitionModuleName(module), path)

    fun isEmpty(): Boolean = module.module.isEmpty() && module.chain == null && path.isEmpty()

    fun subName(name: Name): C_DefinitionName {
...
class C_CommonDefinitionBase(
    ...
    private val mountName: MountName?,
    ...
    fun userBase(pos: S_Pos, executor: C_CompilerExecutor): C_UserDefinitionBase {
    ...
    companion object {
        fun ideId(
            ...
            member: Pair<IdeSymbolCategory, Name>?,
        ): IdeSymbolId {
...
class C_UserDefinitionBase(
    ...
    fun setDocDeclaration(declaration: DocDeclarationProto) {
