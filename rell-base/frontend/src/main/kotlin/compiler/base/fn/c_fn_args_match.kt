    ...
    init {
        ...
        if (last != null) {
            ...
            if (last.arity == M_ParamArity.ONE_MANY) {
                checkEquals(zeroOneCount, 0) { "Invalid parameters: one-many after zero-one $all" }
                mandatoryCount += 1
            }
        }
...
object C_ArgMatcher {
    ...
    private fun bindNamedArg(
        ...
        val param = params.all.find { it.name == name.rName }
