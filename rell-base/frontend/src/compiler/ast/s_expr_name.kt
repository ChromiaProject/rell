...
class S_NameExpr(val qName: S_QualifiedName): S_Expr(qName.pos) {
    ...
    override fun compileWhenEnum(ctx: C_ExprContext, type: R_EnumType): C_Expr {
        ...
        val rrValue = RR_ConstantValue.Enum(
            enumDefIndex = 0, // resolved later during RR resolve
            enumValue = attr.value,
            enumTypeName = type.name,
            enumAttrName = attr.name,
        )
        val vExpr = V_ConstantValueExpr(ctx, startPos, rrValue, type)
...
internal class S_AttrExpr(pos: S_Pos, private val name: S_Name): S_Expr(pos) {
    ...
    companion object {
        ...
        private fun findMembers(ctx: C_ExprContext, hint: C_ExprHint, name: C_Name): ImmList<C_AtContextMember> {
...
internal class S_MemberExpr(val base: S_Expr, val name: S_Name): S_Expr(base.startPos) {
    ...
    private fun compileNullMember(ctx: C_ExprContext, cNameHand: C_NameHandle): C_Expr? {
        ...
        val members = C_LibBridge.instance.getNullExtMembers(cName.rName)
