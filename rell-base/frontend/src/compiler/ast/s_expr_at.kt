...
internal class S_AtExprWhat_Complex(
    ...
    private fun compileSummarizationSum(pos: C_AtSummarizationPos, type: R_Type): C_AtSummarization? {
        return when (type) {
            R_IntegerType -> C_AtSummarization_Aggregate_Sum(pos, type, R_BinaryOp_Add_Integer, RR_ConstantValue.Int(0))
            R_BigIntegerType -> C_AtSummarization_Aggregate_Sum(pos, type, R_BinaryOp_Add_BigInteger, RR_ConstantValue.BigInteger("0"))
            R_DecimalType -> C_AtSummarization_Aggregate_Sum(pos, type, R_BinaryOp_Add_Decimal, RR_ConstantValue.Decimal("0"))
            else -> null
        }
    }
    ...
    private fun compileSummarizationMinMax(
        ...
        val isComparable = type !is R_NullableType && type.isComparable()
        return if (rCmpType == null && !isComparable) null else {
            C_AtSummarization_Aggregate_MinMax(pos, type, cmpOp, rCmpType, dbFn)
        }
    }
...
internal class S_AtExprWhere(
    ...
    private fun makeWhere(ctx: C_ExprContext, compiledExprs: List<V_Expr>): V_Expr? {
        ...
        return if (value == RR_ConstantValue.Bool(true)) null else vExpr
    }
...
internal class S_AtExpr(
    ...
    private fun compileAtResult(whatFields: List<V_DbAtWhatField>): C_AtExprResult {
        ...
        val recordType = if (selFields.size == 1 && selFields[0].name == null) {
            selFields[0].resultType
