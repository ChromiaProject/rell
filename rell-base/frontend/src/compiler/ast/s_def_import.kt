...
internal class C_ImportModulePathHandle(
    ...
    private val pathModuleNames: ImmList<ModuleName>,
) {
    init {
        checkEquals(pathModuleNames.size, nameHand?.parts?.size ?: 0)
    }

    fun resolveIdeInfo(target: C_ImportTarget, moduleInfos: Map<ModuleName, C_ModuleInfo>) {
        nameHand ?: return
...
internal class S_RelativeImportModulePath(val pos: S_Pos, val ups: Int)

internal class S_ImportModulePath(
    ...
    private fun makeModuleNameDetails(base: List<Name>, path: List<Name>): ModuleNameDetails {
        val moduleName = ModuleName.of(base + path)
        val pathNames = path.indices.mapToImmList { ModuleName.of(base + path.subList(0, it + 1)) }
        return ModuleNameDetails(moduleName, pathNames)
    }

    private class ModuleNameDetails(val moduleName: ModuleName, val pathModuleNames: ImmList<ModuleName>)
}

class C_ImportAlias(val explicit: C_Name?, val implicit: C_Name?, val anonymous: C_Name?) {
    init {
        checkNotNull(explicit ?: implicit ?: anonymous)
        if (anonymous != null) {
            checkNull(explicit)
            checkNull(implicit)
        }
...
class S_ExactImportTargetItem(
    ...
    private fun addToNsWildcard(ctx: C_MountContext, nsBuilder: C_UserNsProtoBuilder, details: Details) {
        ...
        val nsBuilder2 = if (aliasHand == null) nsBuilder else {
            ...
            val fullName = FullName(details.currentModuleName, qualifiedName)
    ...
    companion object {
        fun makeAliasIdeId(fullName: QualifiedName, alias: C_Name, category: IdeSymbolCategory): IdeSymbolGlobalId {
