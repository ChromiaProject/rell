...
abstract class S_Pos: Comparable<S_Pos> {
    ...
    fun toFilePos() = FilePos(path().str(), line())
    fun toErrorPos() = ErrorPos(path().str(), line())
...
class S_Name(val pos: S_Pos, private val rName: Name): S_Node() {
    ...
    fun getRNameSpecial(): Name {
        // This method shall be called only in special cases. Whenever possible, one of compile(...) methods must be
        // used to add the name to the context and attach IDE meta-information to the name.
        return rName
    }
    ...
    companion object {
        private val OLD_KEYWORDS: ImmMap<Name, C_ReservedName> =
            mapOf(
                "list" to "0.11.0",
                "set" to "0.11.0",
                "map" to "0.11.0",
            )
            .mapKeys { Name.of(it.key) }
            .mapValuesToImmMap {
                val name = it.key.str
        ...
        private val NEW_KEYWORDS: ImmMap<Name, C_ReservedName> =
            mapOf(
        ...
        private val RESERVED_NAMES: Map<Name, C_ReservedName> = OLD_KEYWORDS.unionNoConflicts(NEW_KEYWORDS)

        private class C_ReservedName(val oldRestrictions: C_FeatureRestrictions?, val newVersion: R_LangVersion?) {
            fun access(ctx: C_NameContext, pos: S_Pos, rName: Name) {
