...
object C_LibTypeAdapter {
    ...
    fun makeTypeBody(
        ...
        namingFactory: (Name) -> C_MemberNaming,
    ): C_LibTypeBody {
    ...
    ): Item = cache.getOrPut(member) { convertTypeMember(typeName, member, cache)}
...
private class C_LibTypeBodyBuilder(
    private val typeName: FullName,
    private val namingFactory: (Name) -> C_MemberNaming,
    ...
    private val valueFunctions = mutableMultimapOf<Name, C_LibTypeItem<L_Function>>()
    private val staticFunctions = mutableMultimapOf<Name, C_LibTypeItem<L_Function>>()
    ...
    private fun checkTypeDef() {
        checkNotNull(typeDef) { "Cannot add a constructor without a typedef" }
    }
