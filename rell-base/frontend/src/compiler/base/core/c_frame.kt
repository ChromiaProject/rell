...
class C_BlockScope(val localVars: ImmMap<Name, C_BlockScopeVar>) {
...
class C_BlockScopeBuilder(
    ...
    private val explicitEntries = mutableMapOf<Name, C_BlockEntry>()
    private val implicitEntries = mutableMultimapOf<Name, C_BlockEntry>()
    ...
    fun lookupVar(name: Name): C_LocalVar? {
        val entry = explicitEntries[name]
        return entry?.toLocalVarOpt()
    }

    fun lookupExplicit(name: Name): C_BlockEntry? {
    ...
    fun addEntry(name: Name, explicit: Boolean, entry: C_BlockEntry) {
        check(!done)
    ...
    private fun ideAddCompletion(name: Name, entry: C_BlockEntry) {
...
sealed class C_BlockContext(
    ...
    abstract fun lookupEntry(name: Name): C_BlockEntryResolution?
    abstract fun lookupLocalVar(name: Name): C_LocalVarRef?
...
class C_OwnerBlockContext(
    ...
    override fun lookupLocalVar(name: Name): C_LocalVarRef? {
        val localVar = findValue { it.scopeBuilder.lookupVar(name) }
        return localVar?.toRef(blockUid)
    }

    override fun lookupEntry(name: Name): C_BlockEntryResolution? {
    ...
    private fun checkNameConflict(pos: S_Pos, name: Name, errOnNameConflict: Boolean = true): Boolean {
...
class C_LambdaBlockBuilder(ctx: C_ExprContext, private val varType: R_Type) {
    ...
    fun build(): C_LambdaBlock {
        ...
        val rLambda = R_LambdaBlock(cBlock.rBlock, varRef.ptr)
