...
class C_GlobalContext(
    ...
    companion object {
        private val appUidGen = C_UidGen { id, _ -> AppUid(id) }
        fun nextAppUid(): AppUid = synchronized(appUidGen) { appUidGen.next("") }
    }
}
...
class C_ModuleProvider(
    ...
    fun getModule(name: ModuleName, extChain: C_ExternalChain?): C_ModuleDescriptor? {
...
sealed class C_ModuleContext(
    ...
    abstract val mountName: MountName
    ...
    val rModuleKey = ModuleKey(moduleName, extChain?.name)
...
class C_ReplModuleContext(
    ...
    override val mountName = MountName.EMPTY
...
class C_MountContext(
    ...
    fun mountName(modMount: C_ModifierValue<C_RawMountAnnotationValue>, simpleName: C_Name): MountName {
        val mountValue = modMount.value()?.process(false)
        return mountName(mountValue, C_QualifiedName(simpleName))
    }

    fun mountName(mountAnn: C_MountAnnotationValue?, qualifiedName: C_QualifiedName?): MountName {
        if (mountAnn != null) {
