...
class C_Name private constructor(val pos: S_Pos, val rName: Name) {
    val str = rName.str

    override fun toString() = str

    companion object {
        fun make(pos: S_Pos, str: String): C_Name = C_Name(pos, Name.of(str))
        fun make(pos: S_Pos, rName: Name): C_Name = C_Name(pos, rName)
    }
}
...
class C_QualifiedName(val parts: ImmList<C_Name>) {
    ...
    fun toRName() = QualifiedName(parts.mapToImmList { it.rName })
...
sealed class C_NameHandle(val pos: S_Pos, val rName: Name): C_IdeSymbolInfoHandle {
    val str = rName.str
...
class C_QualifiedNameHandle(parts: List<C_NameHandle>) {
    ...
    val rName: QualifiedName by lazy {
        QualifiedName(this.parts.mapToImmList { it.rName })
    }
...
sealed class C_NameContext(val msgMgr: C_MessageManager, val compilerOptions: C_CompilerOptions) {
    abstract fun addName(sName: S_Name, rName: Name): C_NameHandle
...
private class C_NameContext_Nop(
    ...
    override fun addName(sName: S_Name, rName: Name): C_NameHandle {
    ...
    private class C_NameHandle_Nop(pos: S_Pos, rName: Name): C_NameHandle(pos, rName) {
        override fun setIdeInfo(ideInfo: C_IdeSymbolInfo) {
            // Do nothing.
        }
...
private class C_NameContext_Active(
    ...
    private fun checkDefIdConflicts(map: Map<S_Pos, IdeSymbolInfo>) {
        ...
        for ((pos, info) in map) {
            if (info.defId != null) {
                ...
                checkNull(oldPos) { "$oldPos $pos ${info.defId}" }
            }
        }
    }
    ...
    private class C_NameHandle_Active(pos: S_Pos, rName: Name): C_NameHandle(pos, rName) {
