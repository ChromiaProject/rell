...
abstract class C_AtFrom(
    ...
    private fun compileColSorting(cFields: List<V_DbAtWhatField>): ImmList<R_ColAtSortEntry> {
        return cFields
                .withIndex()
                .mapNotNullToImmList { (i, f) ->
                    ...
                    if (sort == null) null else {
                        ...
                        if (type.isComparable()) {
                            R_ColAtSortEntry(i, sort.value.asc)
                        } else {
...
class C_AtSummarization_Aggregate_Sum(
    ...
    private val zeroValue: RR_ConstantValue,
...
class C_AtSummarization_Aggregate_MinMax(
    ...
    override fun compileR(appCtx: C_AppContext): R_ColAtFieldSummarization {
        return if (!valueType.isComparable()) {
            typeError(appCtx.msgCtx, valueType, pos)
            R_ColAtFieldSummarization_None
        } else {
            R_ColAtFieldSummarization_Aggregate_MinMax(rCmpOp)
        }
    }
