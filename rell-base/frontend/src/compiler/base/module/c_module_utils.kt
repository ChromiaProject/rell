...
object C_ModuleUtils {
    ...
    private val RELL_MODULE = ModuleName.of("rell")

    fun getModuleInfo(path: C_SourcePath, ast: S_RellFile): Pair<ModuleName?, Boolean> {
        ...
        val rNames = nameParts.mapNotNull { Name.ofOpt(it) }
        if (rNames.size != nameParts.size) {
            return Pair(null, false)
        }

        val moduleName = ModuleName.of(rNames)
        return Pair(moduleName, directory)
    }

    fun isAllowedModuleName(name: ModuleName) = !name.startsWith(RELL_MODULE)
}
...
class C_ModuleReader(
    ...
    private val cache = mutableMapOf<ModuleName, CacheEntry>()

    fun fileSubModules(moduleName: ModuleName) = dirTree.fileSubModules(moduleName)
    fun dirSubModules(moduleName: ModuleName) = dirTree.dirSubModules(moduleName)

    fun readModuleSource(name: ModuleName): C_ModuleSource? {
...
private class C_ModuleDirTree(
    ...
    private val rootDir = DirNode(C_SourcePath.EMPTY, ModuleName.EMPTY)

    fun dirExists(moduleName: ModuleName): Boolean {
        val dirNode = getDirNode(moduleName.parts)
        return dirNode != null
    }

    fun fileSubModules(moduleName: ModuleName): ImmList<C_ModuleSource> {
        val dirNode = getDirNode(moduleName.parts)
        return dirNode?.fileSubModules().orEmpty()
    }

    fun dirSubModules(moduleName: ModuleName): ImmList<ModuleName> {
        val dirNode = getDirNode(moduleName.parts)
    ...
    private fun getDirNode(path: List<Name>): DirNode? {
        var res = rootDir
        for (name in path) {
    ...
    private inner class DirNode(
        ...
        private val subDirs = mutableMapOf<Name, DirNode>()
        ...
        private val dirSubModulesLazy: ImmList<ModuleName> by lazy {
            calcDirSubModules()
        }
        ...
        fun subFile(name: Name): FileNode? {
        ...
        private fun calcAllFileNodes() {
            for (file in rellFilesLazy) {
                if (file !in subFiles) {
                    val rName = Name.ofOpt(file.removeSuffix(C_ModuleUtils.FILE_SUFFIX))
        ...
        private fun calcDirSubModules(): ImmList<ModuleName> {
            val dirs = sourceDir.dirs(path).sorted()

            for (dir in dirs) {
                val rName = Name.ofOpt(dir)
        ...
        private fun makeFileNode(name: String, rName: Name?): FileNode? {
    ...
    private inner class FileNode(
        path: C_SourcePath,
        private val dirModuleName: ModuleName,
        val fileName: String,
        private val rName: Name?,
...
class S_AppContext(
    ...
    fun <T> withModuleContext(moduleName: ModuleName, code: (S_ModuleContext) -> T): T {
...
private class S_PrivateModuleContext(
    ...
    private val namespaces = mutableMultimapOf<QualifiedName, NamespaceNameInfoRec>()
...
sealed class C_ModuleSource(protected val appCtx: S_AppContext, val moduleName: ModuleName) {
    ...
    protected abstract fun compile0(modCtx: S_ModuleContext): ImmList<C_MidModuleFile>
...
class C_FileModuleSource(
    ...
    override fun compile0(modCtx: S_ModuleContext): ImmList<C_MidModuleFile> {
        val compiled = file.compile(modCtx)
        return immListOf(compiled)
    }
}
...
class C_DirModuleSource(
    ...
    override fun compile0(modCtx: S_ModuleContext): ImmList<C_MidModuleFile> {
        return files.mapToImmList { it.compile(modCtx) }
    }
}
