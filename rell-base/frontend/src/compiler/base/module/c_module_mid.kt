...
class C_MidModuleMember_Namespace(
    ...
    private fun makeDocSymbol(
        ...
        val rFullName = FullName(ctx.modCtx.moduleName, qName)
...
class C_MidModuleCompiler(
    ...
    fun compileModule(moduleName: ModuleName, extChain: C_ExtChainName?) {
        check(!done)
        importModule(moduleName, extChain)
        processQueue()
    }

    fun compileReplMembers(moduleName: ModuleName, members: List<C_MidModuleMember>): ImmList<C_ExtModuleMember> {
        check(!done)
    ...
    private fun importModule(name: ModuleName, extChain: C_ExtChainName?) {
        check(!done)
    ...
    private inner class C_MidModuleImporter_Impl: C_MidModuleImporter() {
        override fun importModule(name: ModuleName, extChainName: C_ExtChainName?) {
            this@C_MidModuleCompiler.importModule(name, extChainName)
        }
    }

    private data class C_ExtModuleKey(val name: ModuleName, val chain: C_ExtChainName?)
}

abstract class C_MidModuleImporter {
    abstract fun importModule(name: ModuleName, extChainName: C_ExtChainName?)
}
