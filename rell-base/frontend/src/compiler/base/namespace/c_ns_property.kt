...
class C_NamespaceProperty_RtValue(
    rrValue: Lazy<RR_ConstantValue>,
    private val valueType: R_Type,
    varId: C_VarId?,
): C_NamespaceProperty() {
    private val rrValue by rrValue
    ...
    override fun toExpr(ctx: C_NamespacePropertyContext, name: C_QualifiedName): V_Expr {
        val vExpr = V_ValuePropertyExpr(ctx.exprCtx, name.pos, rrValue, valueType, varKey)
    ...
    private class V_ValuePropertyExpr(
        ...
        pos: S_Pos,
        private val rrValue: RR_ConstantValue,
        private val valueType: R_Type,
        private val varKey: C_VarStateKey?,
    ): V_Expr(exprCtx, pos) {
        override fun exprInfo0() = V_ExprInfo.simple(valueType)
        override fun toRExpr() = R_RRConstantValueExpr(type, rrValue)
        override fun constantValue(ctx: V_ConstantValueEvalContext) = rrValue
