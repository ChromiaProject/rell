...
class C_NsImp_Namespace(val directDefs: ImmMap<Name, C_NsImp_Def>, val importDefs: ImmMultimap<Name, C_NsImp_Def>) {
...
private class C_NsImp_InternalImportsProcessor(
    ...
    private fun processNamespace0(targetNs: C_NsAsm_Namespace, state: C_NsImp_NamespaceState) {
        ...
        state.namespace = builder.build()
    }

    private fun processDirectDef(builder: C_NsImp_NamespaceBuilder, name: Name, def: C_NsAsm_Def) {
        return when (def) {
    ...
    private fun processDirectExactImport(builder: C_NsImp_NamespaceBuilder, name: Name, impDef: C_NsAsm_Def_ExactImport) {
        val imp = impDef.imp
    ...
    private fun processImportDef(builder: C_NsImp_NamespaceBuilder, name: Name, def: C_NsAsm_Def): Unit = when (def) {
        is C_NsAsm_Def_Simple -> {
    ...
    private fun processImportNamespaceDef(builder: C_NsImp_NamespaceBuilder, name: Name, def: C_NsAsm_Def_Namespace) {
        val nsDef = processNamespaceDef(def)
        builder.addImportDef(name, nsDef)
    }

    private fun processNamespaceDef(def: C_NsAsm_Def_Namespace): C_NsImp_Def_Namespace {
        val nsState = processNamespace(def.ns())
        return nsDefs.getOrPut(def) {
            val ideInfo = def.ideSymbolInfo()
            C_NsImp_Def_Namespace({ nsState.namespace }, def.defName, ideInfo, def.deprecated, def.importModule)
        }
    }
...
private class C_NsImp_NamespaceBuilder {
    private val directDefs = mutableMapOf<Name, C_NsImp_Def>()
    private val importDefs = mutableMultimapOf<Name, C_NsImp_Def>()

    fun addDirectDef(name: Name, def: C_NsImp_Def) {
        check(name !in directDefs)
        directDefs[name] = def
    }

    fun addDirectDef(name: Name, member: C_NamespaceMember) {
        val def = C_NsImp_Def_Simple(member)
        addDirectDef(name, def)
    }

    fun addImportDef(name: Name, def: C_NsImp_Def) {
        importDefs.put(name, def)
    }

    fun addImportDef(name: Name, member: C_NamespaceMember) {
        val def = C_NsImp_Def_Simple(member)
        addImportDef(name, def)
    }

    fun build(): C_NsImp_Namespace = C_NsImp_Namespace(directDefs.toImmMap(), importDefs.toImmMultimap())
}
...
private class C_NsImp_ImportResolver(
    ...
    private data class NsNameKey(val ns: C_NsAsm_Namespace, val name: Name)
