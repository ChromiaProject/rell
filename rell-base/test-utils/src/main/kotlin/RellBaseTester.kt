...
abstract class RellBaseTester(
    ...
    private lateinit var rrApp: RR_App
    protected var compilationSysFns: Map<String, Any> = emptyMap()

    val messages = mutableListOf<C_Message>()
    ...
    private fun initCompile(code: String): RR_App {
        ...
        return RellTestUtils.maybeRoundTrip(cRes.rrApp!!)
    }
    ...
    fun moduleSelection(appModules: List<ModuleName>?, testModules: List<ModuleName> = listOf()) {
        moduleSelection(C_CompilerModuleSelection(appModules?.toImmList(), testModules.toImmList()))
    }

    private fun mainModules() = (mainModules ?: listOf("")).mapToImmList { ModuleName.of(it) }
    private fun testModules() = testModules.orEmpty().mapToImmList { ModuleName.of(it) }

    fun moduleSelection(): C_CompilerModuleSelection =
        modSelection ?: C_CompilerModuleSelection(mainModules(), testModules())
    ...
    private fun initSql(moduleCode: String, rrApp: RR_App) {
        tstCtx.sqlMgr().transaction { sqlExec ->
            initSqlReset(sqlExec, moduleCode, rrApp)
            initSqlInserts(sqlExec)
        }
    }

    protected abstract fun initSqlReset(sqlExec: SqlExecutor, moduleCode: String, rrApp: RR_App)
    ...
    private fun dumpDatabaseEntities(): List<String> {
        ...
        val res = tstCtx.innerSqlMgr().access { sqlExec ->
            SqlTestUtils.dumpDatabaseEntity(sqlExec, sqlMapping, rrApp)
        }
        return res
    }
    ...
    fun compileAppEx(code: String): RR_App {
        var res: RR_App? = null
