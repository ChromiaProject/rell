...
object SqlTestUtils {
    ...
    fun dumpDatabaseEntity(sqlExec: SqlExecutor, chainMapping: Rt_ChainSqlMapping, rrApp: RR_App): ImmList<String> {
        val list = mutableListOf<String>()

        for (entity in rrApp.sqlDefs.entities) {
    ...
    private fun dumpEntity(
        ...
        val table = "c${chainMapping.chainId}.${entity.sqlMapping.mountName.str()}"
        val cols = listOf(entity.sqlMapping.rowidColumn) + entity.strAttributes.values.map { it.sqlMapping }
        val sql = getTableDumpSql(table, cols, entity.sqlMapping.rowidColumn)
        val rows = dumpSql(sqlExec, sql).map { "${entity.base.defName.qualifiedName}($it)" }
        list += rows
    }
