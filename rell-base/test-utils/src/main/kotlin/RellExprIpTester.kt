...
class RellExprIpTester: RellExprTester() {
    ...
    private fun calcExpr(expr: String, args: List<TstVal>): String {
        ...
        val res = processExpr0(expr2, types) { app ->
            ...
            val appCtx = Rt_AppContext(globalCtx, Rt_ChainContext.NULL, interpreter, repl = false, test = false)
    ...
    private fun processExpr0(expr: String, types: List<String>, block: (T_App) -> String): String {
        ...
        val res = RellTestUtils.processApp(code, processor = { block(it) })
        return res
    }
    ...
    private class ValCtx(val rrApp: RR_App, val interpreter: Rt_Interpreter)
    ...
    private sealed class InterpTstVal(val type: String): TstVal() {
        ...
        class Obj(val ent: String, val id: Long): InterpTstVal(ent) {
            override fun rt(c: ValCtx): Rt_Value {
                val rrEntity = findEntity(c.rrApp, ent)
                return Rt_EntityValue(c.interpreter.resolveType(rrEntity.type), id)
            }

            private fun findEntity(rrApp: RR_App, name: String): net.postchain.rell.base.model.rr.RR_EntityDefinition {
