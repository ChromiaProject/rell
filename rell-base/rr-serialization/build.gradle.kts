...
// Schema fingerprint: SHA-256 of all .fbs files, emitted as a Kotlin constant.
// Written to App.schema_hash on serialize and checked on deserialize.
...
val generateSchemaHash = tasks.register("generateSchemaHash") {
    ...
    val fbsSrc = fbsDir
    val outDir = generatedSchemaHashDir

    inputs.dir(fbsSrc)
    outputs.dir(outDir)

    doLast {
        // Stable byte array of filename + NUL + file bytes from sorted schema files.
        val digest = MessageDigest.getInstance("SHA-256")
        fbsSrc.asFile.listFiles { f -> f.extension == "fbs" }!!
            .sortedBy { it.name }
            .forEach { file ->
        ...
        val out = outDir.get().asFile.apply { deleteRecursively(); mkdirs() }
        val pkgDir = File(out, "net/postchain/rell/serialization").apply { mkdirs() }
        val bytesLiteral = hash.joinToString(", ") { "0x%02X.toByte()".format(it) }

        File(pkgDir, "FbsSchemaHash.kt").writeText(
            """
            // Auto-generated by the generateSchemaHash Gradle task. Do not edit.
            package net.postchain.rell.serialization

            /**
             * SHA-256 of all `.fbs` schema files in this module, computed at build time.
             * Written to `App.schema_hash` on serialize and checked on deserialize.
             * The C++ runtime mirrors this computation at its build time and verifies
             * the hash when loading buffers, so wire-format drift surfaces as a clean
             * load-time error rather than as corrupted deserialization downstream.
             */
            val FBS_SCHEMA_HASH: ByteArray = byteArrayOf(
                $bytesLiteral
            )
            """.trimIndent(),
        )
...
sourceSets.main {
    ...
    kotlin.srcDir(generatedSchemaHashDir)
}
...
dependencies {
    ...
    implementation(libs.gtv.min.core)
