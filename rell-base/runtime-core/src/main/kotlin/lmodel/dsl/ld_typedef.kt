...
sealed class Ld_TypeDefMember(
    ...
    fun finish(ctx: Ld_TypeFinishContext, typeName: FullName): List<L_TypeDefMember> {
...
private class Ld_TypeDefMember_Function(
    ...
    private fun makeAliasDocSymbol(
        ...
        val docDec = DocDeclarationProto_Alias(
                ...
                QualifiedName.of(listOf(targetFullName.last)),
            )
            .toLazyDeclaration()
...
class Ld_TypeDef(
    ...
    private fun finish(ctx: Ld_NamespaceFinishContext, hdr: Ld_MemberHeader.Finish): Result {
        ...
        val effectiveRTypeMeta = rTypeMeta ?: rrType?.let { L_TypeUtils.rrTypeToRTypeMeta(it) }
        val mGenericType = makeGenericType(fullName, lTypeParams.list, mParent, effectiveRTypeMeta)
    ...
    private fun makeGenericType(
        ...
        effectiveRTypeMeta: R_TypeMeta?,
    ): M_GenericType {
        if (mTypeParams.isEmpty() && effectiveRTypeMeta != null) {
            val rType = effectiveRTypeMeta.getTypeOrNull(immListOf())
            if (rType != null) {
...
internal class Ld_TypeDefDslImpl<T : Rt_Value>(
    ...
    override fun rrType(rrType: RR_Type) {
        maker.rrType(rrType)
    }
...
private class Ld_TypeDefBuilder<T : Rt_Value>(
    ...
    private var rrTypeValue: RR_Type? = null
    ...
    override fun generic(
        ...
        checkNull(selfType) { "Trying to add a type parameter after self type was requested" }
        checkNull(parentType) { "Trying to add a type parameter after a parent type" }
        checkNull(rTypeMeta) { "Trying to add a type parameter after R_Type" }
    ...
    override fun parent(type: String) {
        checkNull(parentType) { "Parent type already set" }
    ...
    override fun rrType(rrType: RR_Type) {
        checkNull(rrTypeValue) { "RR_Type already set" }
        checkNull(rTypeMeta) { "R_Type already set" }
        check(members.isEmpty()) { "Trying to set RR_Type after a member" }
        rrTypeValue = rrType
    }
    ...
    override fun docCodeStrategy(strategy: L_TypeDefDocCodeStrategy) {
        checkNull(docCodeStrategy) { "strCode strategy already set" }
    ...
    override fun supertypeStrategy(strategy: L_TypeDefSupertypeStrategy) {
        checkNull(supertypeStrategy) { "Supertype strategy already set" }
