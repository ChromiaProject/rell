...
class Ld_FullName(val moduleName: ModuleName?, val qualifiedName: QualifiedName) {
    override fun toString(): String =
        if (moduleName == null) qualifiedName.str() else "${moduleName.str()}${MODULE_SEP}${qualifiedName.str()}"
    ...
    companion object {
        ...
        fun parse(s: String): Ld_FullName {
            ...
            return if (i < 0) {
                val qName = QualifiedName.of(s)
                Ld_FullName(null, qName)
            } else {
                val moduleName = ModuleName.of(s.substring(0, i))
                val qName = QualifiedName.of(s.substring(i + MODULE_SEP.length))
...
class Ld_Alias(val memberHeader: Ld_MemberHeader, val simpleName: Name, val deprecated: C_Deprecated?) {
    companion object {
        fun make(
...
class Ld_AliasesBuilder(private val primaryName: Name) {
    private val aliases = mutableMapOf<Name, Ld_Alias>()
...
internal class Ld_MemberConflictChecker(initialNames: Map<Name, Ld_ConflictMemberKind>) {
    ...
    fun finish(): ImmMap<Name, Ld_ConflictMemberKind> {
