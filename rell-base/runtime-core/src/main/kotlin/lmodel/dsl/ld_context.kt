...
class Ld_ModuleContext(
    ...
    private val members = mutableMultimapOf<QualifiedName, FcFuture<List<L_NamespaceMember>>>()
    private var finished = false

    fun declareMember(qualifiedName: QualifiedName, future: FcFuture<List<L_NamespaceMember>>) {
        members.put(qualifiedName, future)
    }

    fun finish(imports: ImmMap<ModuleName, L_Module>): Ld_NamespaceFinishContext {
        check(!finished)
        finished = true
...
class Ld_NamespaceContext(
    ...
    fun getFullName(simpleName: Name): FullName {
        return namePath.fullName(simpleName)
    }

    fun nestedNamespaceContext(simpleName: Name): Ld_NamespaceContext {
...
class Ld_ModuleFinishContext(
    ...
    val imports: ImmMap<ModuleName, L_Module>,
    val members: ImmMap<QualifiedName, FcFuture<List<L_NamespaceMember>>>,
)
...
class Ld_NamespaceFinishContext(
    ...
    private fun getTypeDefOrNull(fullName: Ld_FullName): L_AbstractTypeDef? {
        ...
        val found = mutableListOf<Pair<L_AbstractTypeDef, ModuleName?>>()
    ...
    private fun handleTypeRecursion(qualifiedName: QualifiedName, e: FcCycleException): Nothing {
        var cycle = e.nodes.mapNotNull { it.attachment as? FullName }
        ...
        if (cycle.isEmpty()) {
            val fullName = FullName(modCtx.moduleName, qualifiedName)
    ...
    private fun <T> checkTypeAmbiguity(qualifiedName: QualifiedName, types: List<T>, moduleGetter: (T) -> String) {
    ...
    fun getNamespaceMembers(qualifiedName: QualifiedName): List<L_NamespaceMember> {
...
class Ld_TypeFinishContext(
        ...
        private val typeParams: ImmMap<Name, M_Type>,
) {
    val modCfg = defCtx.modCfg

    fun subCtx(typeParams: Map<Name, M_Type>): Ld_TypeFinishContext {
