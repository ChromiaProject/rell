...
class Ld_CommonNamespaceDslImpl(
    ...
    private fun constant0(
        ...
        val ldValue = Ld_ConstantValue.Value(value)
...
class Ld_NamespaceBuilder(
    ...
    private val namespaces: MutableMap<Name, Ld_Namespace> = baseNamespace.namespaces.toMutableMap()
    ...
    override fun include(namespace: Ld_Namespace) {
        ...
        for ((simpleName, ns) in namespace.namespaces) {
            namespace0(QualifiedName.of(listOf(simpleName))) { subBuilder ->
    ...
    override fun namespace(name: String, hdr: Ld_MemberHeader, block: Ld_NamespaceDsl.() -> Unit) {
        val qName = QualifiedName.of(name)
    ...
    private fun namespace0(namespaceName: QualifiedName, block: (Ld_NamespaceBuilder) -> Unit) {
        ...
        if (namespaceName.size() > 1) {
            val subQualifiedName = QualifiedName(namespaceName.parts.drop(1).toImmList())
    ...
    override fun alias(
        ...
        val targetName = QualifiedName.of(target)
    ...
    private fun getSimpleName(name: String): Name {
        return Name.of(name)
    }
...
class Ld_Namespace(
    ...
    val namespaces: ImmMap<Name, Ld_Namespace>,
    val members: ImmList<Ld_NamespaceMember>,
    val nameKinds: ImmMap<Name, Ld_ConflictMemberKind>,
) {
...
private class Ld_NamespaceMember_Alias(
    ...
    private val targetName: QualifiedName,
