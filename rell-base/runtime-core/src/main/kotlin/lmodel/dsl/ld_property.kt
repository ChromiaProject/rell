...
sealed interface Ld_PropertyValue {
    ...
    companion object {
        ...
        fun typeProp(pure: Boolean, valueGetter: (Rt_Value, R_Type) -> Rt_Value): Ld_PropertyValue {
            return Ld_PropertyValue_SimpleTypeProp(pure, valueGetter)
        }

        fun typeProp(body: C_SysFunctionBody): Ld_PropertyValue = Ld_PropertyValue_SpecialTypeProp(body)
    }
}
...
class Ld_NamespacePropertyDslImpl(
    ...
    override fun validate(validator: (C_SysFunctionCtx) -> Unit) {
        checkNull(buildRes) { "Body already set" }
