...
internal object Lib_SysQueries {
...
private object SysQueryFns {
    ...
    object GetBuildDetails: R_SysFunctionEx_N() {
        ...
        override fun call(ctx: Rt_CallContext, args: List<Rt_Value>): Rt_Value {
            checkEquals(args.size, 0)
            val ver = ctx.globalCtx.rellVersion()
            return Rt_MapValue(RT_TYPE, ver.rtProperties.toMutableMap())
        }
    }
    ...
    object GetAppStructure: R_SysFunctionEx_N() {
        override fun call(ctx: Rt_CallContext, args: List<Rt_Value>): Rt_Value {
            checkEquals(args.size, 0)
            val v = ctx.appCtx.interpreter.metaGtv
    ...
    object GetMountNames: R_SysFunctionEx_N() {
        val PARAMS = immListOf(
            R_FunctionParam(Name.of("kinds"), R_ListType(R_TextType)),
            R_FunctionParam(Name.of("modules"), R_ListType(R_TextType)),
        )
        ...
        private fun <T> GtvBuilder.addMountNames(
            kindsArg: Set<String>,
            kind: String,
            resultKey: String,
            definitions: Map<*, T>,
            mountNameGetter: (T) -> MountName,
                )
        {
        ...
        private fun Collection<MountName>.toGtvArrayNode(): GtvBuilder.GtvArrayNode {
    ...
    object GetModuleArgs: R_SysFunctionEx_N() {
        ...
        override fun call(ctx: Rt_CallContext, args: List<Rt_Value>): Rt_Value {
            ...
            val rrApp = ctx.appCtx.rrApp
            val moduleArgsMap = rrApp.moduleArgs
                .filterKeys { modules.containsOrEmpty(it) }
                .mapNotNull { (moduleName, rrStructDef) ->
                    ctx.appCtx.getModuleArgs(moduleName)?.let { moduleArgs ->
                        val structIdx = rrApp.structNameIndex[rrStructDef.struct.name] ?: return@let null
                        val rtType = ctx.appCtx.interpreter.resolveType(RR_Type.Struct(structIdx))
                        val gtvConv = rtType.gtvConversion ?: return@let null
                        val gtv = gtvConv.rtToGtv(moduleArgs, pretty = true)
