...
internal sealed interface Rt_StatementResult {
...
/**
 * Tree-walker activation record: an `Array<Rt_Value?>` plus block-scope bookkeeping. Implements the
 * core-side [Rt_Frame] marker so stdlib helpers can report errors against the current frame without
 * a compile-time dependency on this class. A Truffle backend would supply its own `VirtualFrame`-
 * backed implementation of [Rt_Frame].
 */
...
@JvmInline internal value class Rt_CallFrameState(val values: ImmList<Optional<Rt_Value>>) {
    companion object {
