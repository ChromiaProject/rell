...
class Rt_InterpreterImpl(
    ...
    override val metaGtv: Gtv by lazy { buildMetaGtv() }
    ...
    override fun callFunction(
        ...
        return if (result is Rt_StatementResult.Return) result.value ?: Rt_UnitValue else Rt_UnitValue
    }
    ...
    override fun callQuery(query: RR_QueryDefinition, exeCtx: Rt_ExecutionContext, args: List<Rt_Value>): Rt_Value {
        ...
        return when (val body = query.body) {
            is RR_UserQueryBody -> {
                ...
                if (result is Rt_StatementResult.Return) result.value ?: Rt_UnitValue else Rt_UnitValue
            }
    ...
    override fun evaluateAttributeDefault(
        ...
        if (entityIdx != null) {
            ...
            val frame = createFrame(exeCtx, entity.base.initFrame, dbUpdateAllowed, entity.base.defId)
        ...
        for (structDef in rrApp.allStructs) {
            if (structDef.base.defId == defId) {
                ...
                val frame = createFrame(exeCtx, structDef.base.initFrame, dbUpdateAllowed, structDef.base.defId)
                return evaluateExpr(defaultExpr, frame)
            }
        }
    ...
    fun executeStmt(stmt: RR_Statement, frame: Rt_CallFrame): Rt_StatementResult? = when (stmt) {
        ...
        is RR_Statement.Return -> {
            val value = stmt.expr?.let { evaluateExpr(it, frame) }
            Rt_StatementResult.Return(value)
        }
        ...
        is RR_Statement.While -> {
            while (true) {
                ...
                when (res) {
                    Rt_StatementResult.Break -> break
                    Rt_StatementResult.Continue -> continue
                    is Rt_StatementResult.Return -> return res
                    null -> {}
                }
            }
            null
        ...
        is RR_Statement.Break -> Rt_StatementResult.Break
        is RR_Statement.Continue -> Rt_StatementResult.Continue
        ...
        is RR_Statement.Update -> {
            executeUpdate(stmt, frame)
            null
        }

        is RR_Statement.Delete -> {
            executeDelete(stmt, frame)
            null
        }
    }
     ...
     */
    override fun resolveType(type: RR_Type): Rt_ValueClass<*> = typeCache.getOrPut(type) { buildRtType(type) }

    private val typeCache = mutableMapOf<RR_Type, Rt_ValueClass<*>>()

    /**
     * Resolve an [R_Type] to an [Rt_ValueClass] for this app. Used by stdlib meta-bodies that
     * receive [R_Type] from compiler frontend metadata but need a runtime type-class. For
     * definition-backed kinds (entity/struct/enum/object/operation/virtual_struct), the proper
     * `defIndex` is looked up by app-level name in this app's def lists; primitives and
     * composites resolve recursively.
     */
    override fun resolveRType(rType: R_Type): Rt_ValueClass<*> = resolveType(rrTypeOfR(rType))

    private fun rrTypeOfR(rType: R_Type): RR_Type = when (rType) {
        ...
        is R_NullableType -> RR_Type.Nullable(rrTypeOfR(rType.valueType))
        is R_ListType -> RR_Type.List(rrTypeOfR(rType.elementType))
        is R_SetType -> RR_Type.Set(rrTypeOfR(rType.elementType))
        is R_MapType -> RR_Type.Map(rrTypeOfR(rType.keyType), rrTypeOfR(rType.valueType))
        is R_TupleType -> RR_Type.Tuple(rType.fields.mapToImmList {
            RR_TupleField(it.name?.str, rrTypeOfR(it.type))
        })
        is R_EntityType -> {
            val name = rType.rEntity.appLevelName
            val idx = rrApp.allEntities.indexOfFirst { it.base.appLevelName == name }
            require(idx >= 0) { "Entity not in rrApp: $name" }
            RR_Type.Entity(idx)
        }
        is R_StructType -> {
        ...
        is R_EnumType -> {
            val name = rType.enum.appLevelName
            val idx = rrApp.allEnums.indexOfFirst { it.base.appLevelName == name }
            require(idx >= 0) { "Enum not in rrApp: $name" }
            RR_Type.Enum(idx)
        }
        is R_ObjectType -> {
            val name = rType.rObject.appLevelName
            val idx = rrApp.allObjects.indexOfFirst { it.base.appLevelName == name }
            require(idx >= 0) { "Object not in rrApp: $name" }
            RR_Type.Object(idx)
        }
        is R_OperationType -> {
            val name = rType.rOperation.appLevelName
            val idx = rrApp.allOperations.indexOfFirst { it.base.appLevelName == name }
            require(idx >= 0) { "Operation not in rrApp: $name" }
            RR_Type.Operation(idx)
        }
        is R_VirtualListType -> RR_Type.VirtualList(rrTypeOfR(rType.innerType.elementType))
        is R_VirtualSetType -> RR_Type.VirtualSet(rrTypeOfR(rType.innerType.elementType))
        is R_VirtualMapType -> RR_Type.VirtualMap(
            rrTypeOfR(rType.innerType.keyType),
            rrTypeOfR(rType.innerType.valueType),
        )
        ...
        is R_FunctionType -> RR_Type.Function(
            rType.params.mapToImmList { rrTypeOfR(it) },
            rrTypeOfR(rType.result),
        )
        is R_CtErrorType -> RR_Type.Error
        else -> RR_Type.Generic(rType.name, net.postchain.rell.base.utils.immListOf())
    }
    ...
    override fun callTarget(
        ...
        val callFrame = frame as Rt_CallFrame
        val result = try {
            callTarget0(target, base, args, callFrame)
        } catch (e: Rt_Exception) {
            if (callPos != null) {
                callFrame.error(ErrorPos(callPos), e, true)
            } else {
                throw e
            }
        }
    ...
    private fun callTarget0(
        ...
        is RR_FunctionCallTarget.Extendable -> {
            ...
            for (rrFnBase in extensions) {
                ...
                val value = if (result is Rt_StatementResult.Return) result.value ?: Rt_UnitValue else Rt_UnitValue
...
// AtCardinality.isMany and RR_Type.elementType() moved to runtime-core/runtime/rt_interpreter_helpers.kt
...
// Rt_RR_LazyValue and Rt_RR_EnumValue moved to runtime-core/runtime/rt_value_rr_lazy.kt and rt_value_rr_enum.kt.
