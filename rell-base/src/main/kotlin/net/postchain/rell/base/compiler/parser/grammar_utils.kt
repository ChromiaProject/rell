...
class RellTokenMatch(
    ...
    val comment: S_Comment? = comment?.let { S_Comment(pos, it) }
}

internal class RellTokenParsed(
    private val tokenIndex: Int,
    val token: RellToken,
    val match: RellTokenMatch,
): Parsed<RellTokenMatch>() {
    override val value get() = match
...
internal class RellTokenInput(
    private val text: String,
    val tokenMatch: RellTokenMatch,
    val rellParsed: RellTokenParsed,
...
internal val TokenMatch.rellParsed: RellTokenParsed get() {
    return (input as RellTokenInput).rellParsed
}
