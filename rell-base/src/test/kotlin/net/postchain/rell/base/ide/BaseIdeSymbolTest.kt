...
import net.postchain.rell.base.compiler.parser.rellParsed
...
abstract class BaseIdeSymbolTest: BaseRellTest(false) {
    ...
    companion object {
        ...
        private fun extractSymbols(sourceDir: C_SourceDir, file: C_SourcePath): Map<S_Pos, String> {
            ...
            while (true) {
                val p = tp.nextToken()?.rellParsed ?: break
                val m = p.match
                if (p.token.pattern == RellTokenizer.IDENTIFIER || p.token.pattern == "$") syms[m.pos] = m.text
            }
