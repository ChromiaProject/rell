...
class Rt_Interpreter(
        ...
        return when (val body = query.body) {
            ...
            is RR_SysQueryBody -> {
                val fn = checkNotNull(stdlib.sysFunctions[body.fnName]) {
                    error("Sys query function not found: ${body.fnName}")
                }
     ...
     * no [R_Type] involved.
     */
    private fun buildCompositeGtvConversion(type: RR_Type): Rt_TypeGtvConversion? = when (type) {
     ...
     */
    private fun decodeVirtualElementRR(ctx: GtvToRtContext, innerType: RR_Type, gtv: Gtv): Rt_Value {
    ...
    private fun decodeVirtualDictEntries(ctx: GtvToRtContext, gtv: Gtv): Map<String, Gtv> {
        if (gtv !is GtvVirtual) {
            return try {
                gtv.asDict()
            } catch (_: UserMistake) {
        ...
        if (gtv !is GtvVirtualDictionary) {
            val cls = gtv.javaClass.simpleName
            throw GtvRtUtils.errGtv(ctx, "virtual:deserialized_type:$cls", "Wrong deserialized Gtv type: $cls")
        }
        return gtv.dict
    ...
    private fun decodeVirtualStructFromGtv(ctx: GtvToRtContext, type: RR_Type.Struct, gtv: Gtv): Rt_Value {
    ...
    private fun decodeVirtualListFromGtv(
        ctx: GtvToRtContext,
        elementType: RR_Type,
        gtv: Gtv,
        asSet: Boolean,
    ...
    private fun decodeVirtualMapFromGtv(ctx: GtvToRtContext, mapType: RR_Type.Map, gtv: Gtv): Rt_Value {
    ...
    private fun decodeVirtualTupleFromGtv(ctx: GtvToRtContext, tupleType: RR_Type.Tuple, gtv: Gtv): Rt_Value {
        val gtvFields: List<Gtv?> = decodeVirtualArrayElements(ctx, gtv)
    ...
    private fun buildVirtualStructGtvConversion(type: RR_Type.VirtualStruct): Rt_TypeGtvConversion {
    ...
    private fun buildVirtualListGtvConversion(type: RR_Type.VirtualList): Rt_TypeGtvConversion {
        return object: Rt_TypeGtvConversion {
            override fun rtToGtv(value: Rt_Value, pretty: Boolean): Gtv =
                throw Rt_GtvError.exception("virtual:to_gtv", "Cannot convert virtual to Gtv")
    ...
    private fun buildVirtualSetGtvConversion(type: RR_Type.VirtualSet): Rt_TypeGtvConversion {
        return object: Rt_TypeGtvConversion {
            override fun rtToGtv(value: Rt_Value, pretty: Boolean): Gtv =
                throw Rt_GtvError.exception("virtual:to_gtv", "Cannot convert virtual to Gtv")
    ...
    private fun buildVirtualMapGtvConversion(type: RR_Type.VirtualMap): Rt_TypeGtvConversion {
        return object: Rt_TypeGtvConversion {
            override fun rtToGtv(value: Rt_Value, pretty: Boolean): Gtv =
                throw Rt_GtvError.exception("virtual:to_gtv", "Cannot convert virtual to Gtv")
    ...
    private fun buildVirtualTupleGtvConversion(type: RR_Type.VirtualTuple): Rt_TypeGtvConversion {
        return object: Rt_TypeGtvConversion {
            override fun rtToGtv(value: Rt_Value, pretty: Boolean): Gtv =
                throw Rt_GtvError.exception("virtual:to_gtv", "Cannot convert virtual to Gtv")
    ...
    private fun buildNullableGtvConversion(type: RR_Type.Nullable): Rt_TypeGtvConversion =
    ...
    private fun buildListLikeGtvConversion(
        outerType: RR_Type,
        elementType: RR_Type,
        isSet: Boolean,
    ...
    private fun buildMapGtvConversion(type: RR_Type.Map): Rt_TypeGtvConversion {
        return object: Rt_TypeGtvConversion {
            override fun rtToGtv(value: Rt_Value, pretty: Boolean): Gtv {
    ...
    private fun buildTupleGtvConversion(type: RR_Type.Tuple): Rt_TypeGtvConversion {
        return object: Rt_TypeGtvConversion {
            override fun rtToGtv(value: Rt_Value, pretty: Boolean): Gtv {
    ...
    private fun buildStructGtvConversion(rrType: RR_Type.Struct, structDef: RR_StructDefinition): Rt_TypeGtvConversion {
        return object: Rt_TypeGtvConversion {
            override fun rtToGtv(value: Rt_Value, pretty: Boolean): Gtv {
    ...
    private fun rrTypeName(type: RR_Type): String = when (type) {
    ...
    private fun callTarget0(
        ...
        is RR_FunctionCallTarget.FunctionValue -> {
            ...
            fnValue.call(frame.callCtx(), args)
        }
...
/**
 * Adapter that wraps [RR_FunctionCallTarget] dispatch into [R_FunctionCallTarget]
 * for use with [Rt_FunctionValue] (partial application).
 */
class Rt_FunctionCallTargetAdapter(
 ...
 * Creates a lightweight [R_EnumAttr] bridge from [RR_EnumAttr] data on demand.
 */
internal class Rt_RR_EnumValue(
    ...
    // Bridge R_EnumAttr — used by asEnum() callers that need the enum attribute metadata.
    private val fakeRAttr: R_EnumAttr by lazy {
        R_EnumAttr(rrAttr.rName, rrAttr.value, C_IdeSymbolInfo.UNKNOWN, null)
    }

    override fun type() = rtTypeRef.value
    override fun asEnum() = fakeRAttr
