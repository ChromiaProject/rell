...
class RellHtmlRenderer(
    ...
    private fun createTabsForPackage(page: PackagePage): List<ContentTab> {
        ...
        return buildList {
            if (p.typealiases.isNotEmpty() || p.classlikes.isNotEmpty()) add(
    ...
    private fun FlowContent.buildPlatformDependent(
        ...
        val isOnlyCommonContent = contents.singleOrNull()?.let { (sourceSet, _) ->
            sourceSet.platform == org.jetbrains.dokka.Platform.common
    ...
    override fun OL.buildListItems(
    ...
    override fun UL.buildListItems(
            items: List<ContentNode>,
            pageContext: ContentPage,
            sourceSetRestriction: Set<DisplaySourceSet>?
    ) {
    ...
    /*override fun FlowContent.buildDefaultTable(
            node: ContentTable,
            pageContext: ContentPage,
            sourceSetRestriction: Set<DisplaySourceSet>?
    ) {
...
/*    public fun FlowContent.buildLink(
    ...
    private fun FlowContent.buildText(textNode: ContentText, unappliedStyles: Set<Style>) {
        when {
            textNode.extra[org.jetbrains.dokka.pages.HtmlContent] != null -> {
     ...
     */
    override fun FlowContent.clickableLogo(page: PageNode, pathToRoot: String) {
...
private fun TabbedContentType.toHtmlAttribute(): String =
        when (this) {
            ...
            is RellTabbedContentType ->
                when (this) {
                    RellTabbedContentType.FUNCTION -> "FUNCTION"
                    RellTabbedContentType.QUERY -> "QUERY"
