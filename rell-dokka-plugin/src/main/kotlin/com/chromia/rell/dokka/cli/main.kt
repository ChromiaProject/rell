...
/**
 * CLI entry point — `mainClass = com.chromia.rell.dokka.cli.MainKt`.
 *
 * The flag set is unchanged from the prior Dokka-bound version because chromia-cli uses this
 * binary directly (`./gradlew :rell-dokka-plugin:run --args="--source … --target …"`) and
 * external docs (`work/build-local-docs.sh` etc.) pass these same options.
 */
internal class DokkaCommand : CliktCommand() {
